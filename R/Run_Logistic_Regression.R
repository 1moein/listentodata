...
# Now, load the data sets and view them
d = read.csv(file="emailclickdata.csv")
df = read.csv(file="newemails.csv")
head(d)
str(d)

## Estimate the logistic regression model and show its summary

logit1 = glm(visited ~ ., data = d, family = "binomial")
summ(logit1)

## Predict the probability of visiting
df
clickprobability = predict(logit1, df, type="response")
...
filename =  "Regression Analysis Results_Without_tables.pdf"
larger =  1
pdf(filename, height=larger*8.5, width=larger*11)

# A scatterplot matrix of numeric variables
pairs(d1[,1:8], lower.panel = NULL, pch=16,cex=0.3)

correlations = cor(d1[,1:8])
corrplot(correlations, method="pie")
corrplot(correlations, method="number")

summ(m6)
export_summs(m1, m2, m3, m4, m5, m6)
plot_summs(m6, scale = TRUE, plot.distributions = TRUE, inner_ci_level = .95)

# Display the best model
summ(selected_model)

# Display the best model visually
plot_summs(selected_model, scale = TRUE, plot.distributions = TRUE, inner_ci_level = .95)
